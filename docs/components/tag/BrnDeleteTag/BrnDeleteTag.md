...
| onTagDelete       | Function(List, String, int) | 点击删除某个标签后将现有的所有标签，删除的标签内容，删除的标签 index 回传 | 否           |                                    |
...
| deleteIconSize    | Size                        | 设置删除 Icon 的大小                                                      | 否           | 默认值为内置删除 icon 的图片大小。 |
...
| clear          | Function     | 清空标签                         |
| deleteForIndex | int          | 删除某个 index 底下 tag          |
