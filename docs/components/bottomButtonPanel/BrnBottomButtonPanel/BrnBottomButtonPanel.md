...
1. 常用于页面底部的组合按钮：主题色按钮、辅助色按钮、icon 按钮
...
| iconButtonList        | List         | icon 按钮的集合  | 否       | 无     |
